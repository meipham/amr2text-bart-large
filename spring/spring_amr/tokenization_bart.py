...
class AMRBartTokenizer(BartphoTokenizer):
    # INIT = 'Ġ'
    INIT = '▁'
    ...
    def init_amr_vocabulary(self, pred_min=5):
        ...
        # self.mask_token = self.INIT + self.mask_token
        ...
        self.lit_start = self.INIT + AMRTokens.LIT_START
        self.lit_end = self.INIT + AMRTokens.LIT_END
        
    def _tok_bpe(self, token, add_space=True):
        tok = token.replace('_', ' ')
        return self.tokenize(tok)
    ...
    def tokenize_amr(self, graph):
        ...
        for i, (backr, tokk) in enumerate(zip(backreferences, linearized_nodes)):
            ...
            is_lit_start = tokk.startswith('"')
            is_lit_end = tokk.endswith('"')

            if is_lit_start: tokk = tokk[1::]
            if is_lit_end: tokk = tokk[0:-1]
            
            is_in_enc = self.INIT + tokk in self.encoder # if can encode token
            is_rel = tokk.startswith(':') and len(tokk) > 1 # if is a relationship
            is_of  = tokk.startswith(':') and tokk.endswith('-of') # if is a reverse relationship
            is_frame = re.match(r'.+-\d\d', tokk) is not None # if is a pre-defined frame
            ...
            if is_lit_start: 
                bpe_toks = [self.lit_start] + bpe_toks
            if is_lit_end:
                bpe_toks = bpe_toks + [self.lit_end]
