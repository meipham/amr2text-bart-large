...
class AMRDataset(Dataset):
    ...
    def __init__(
        ...
        for g in graphs:
            ...
            snt = g.metadata['snt'].replace('_', ' ')
            try:
                snt_ids = self.tokenizer.encode(snt)
            except:
        ...
        res_out = [json.dumps({
            "src": sent, "tgt": " ".join(lamr)}, ensure_ascii=False) for sent, lamr in zip(self.sentences, self.linearized_tokens)]

        outpath = f"/AIHCM/KGQA/NLPCore/content-writer/data/{data_cate}/{type_path}.jsonl"
