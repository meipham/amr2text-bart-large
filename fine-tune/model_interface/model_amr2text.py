...
class AMR2TextModelModule(pl.LightningModule):
    ...
    def __init__(self, tokenizer, args):
        ...
        self.train_dataset_size = args.train_dataset_size
    ...
    def setup(self, stage='fit'):
        if stage == "fit":
            if isinstance(self.hparams.gpus, list):
                num_devices = max(1, len(self.hparams.gpus))
            else:
                num_devices = max(1, self.hparams.gpus)
            effective_batch_size = (
                self.hparams.per_gpu_train_batch_size * self.hparams.accumulate_grad_batches * num_devices
            )
            print(f"Effective batch size: {effective_batch_size}")
            if self.hparams.max_steps <= 0:
                self.total_steps = (self.train_dataset_size / effective_batch_size)
                ...
                effective_epochs = self.hparams.max_steps / (
                    self.train_dataset_size / effective_batch_size
                )
